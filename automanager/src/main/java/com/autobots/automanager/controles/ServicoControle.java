...
public class ServicoControle {
	...
	@GetMapping("/servicoPempresa/{id}")
	public ResponseEntity<Set<Servico>> servicoEmpresa (@PathVariable Long id){
		Empresa empresa = listagem.buscarEmpresa(id);
		if(empresa != null) {
			Set<Servico> servicos = empresa.getServicos();
			return new ResponseEntity<Set<Servico>>(servicos, HttpStatus.FOUND);
		}
		else {
			return new ResponseEntity<Set<Servico>>(HttpStatus.NOT_FOUND);
		}
	}
